...
from runtime.decision_sync import reconcile_decision_sources, sync_reply_to_runstate
...
from runtime.reply_parser import create_reply_record, parse_reply, validate_reply
...
def list(
    ...
    if status:
        try:
            ...
            console.print("[yellow]Valid: pending, sent, reply_received, resolved, expired[/yellow]")
            raise typer.Exit(1)
    ...
    console.print("[dim]Use 'asyncdev decision reply --request <id> --command \"DECISION A\"' to resolve[/dim]")
    console.print("[dim]Use 'asyncdev decision wait --request <id>' to poll for reply[/dim]")
...
def reply(
    ...
    if not parsed.is_valid:
        ...
        console.print("[yellow]Valid: DECISION A, DEFER, RETRY, CONTINUE[/yellow]")
        raise typer.Exit(1)
    ...
    if blocking_status == "CLEAR":
        ...
        if recovery_item and recovery_item.recovery_required:
            ...
            console.print("  Decision resolved, but recovery still needed:")
