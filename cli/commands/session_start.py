...
def check(
    ...
    if blocked_projects:
        ...
        console.print("[dim]Run: asyncdev decision wait --request <id>[/dim]")
        console.print("[dim]Or: asyncdev decision reply --request <id> --command \"DECISION X\"[/dim]")
...
def status(
    ...
    if blocked_count > 0 or waiting_count > 0:
        ...
        console.print("[dim]Run: asyncdev session-start check for details[/dim]")
    else:
        console.print("\n[bold green]ALL CLEAR[/bold green]")
        console.print("[dim]Proceed with execution[/dim]")
